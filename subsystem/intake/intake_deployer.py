    ...
    def setup(self) -> None:
        ...
        self.intake_deploy_encoder.configurator.apply(
            phoenix6.configs.CANcoderConfiguration().with_magnet_sensor(
                ...
                .with_sensor_direction(intake_constants.deploy_encoder_direction)
            )
        )
        self.intake_deploy_motor.configurator.apply(
            ...
            .with_feedback(
                ...
                .with_feedback_sensor_source(phoenix6.signals.FeedbackSensorSourceValue.FUSED_CANCODER)
                .with_feedback_remote_sensor_id(intake_constants.deploy_encoder_can_id)
                .with_sensor_to_mechanism_ratio(intake_constants.deploy_sensor_to_mechanism_ratio)
                .with_rotor_to_sensor_ratio(intake_constants.deploy_rotor_to_sensor_ratio)
            )
            .with_motor_output(
                phoenix6.configs.MotorOutputConfigs().with_inverted(
                    intake_constants.deploy_motor_inverted
                )
            )
        )

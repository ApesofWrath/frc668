...
class ShooterStateMachine(StateMachine):
    ...
    def targeting(self,initial_call):
        ...
        if self._get_shooter_state_is_target_state_default() and self.is_shooting and not self.is_drivetrain_moving():
    ...
    def idling(self):
        ...
        self.turret.setPosition(self.hub_tracker.get_target_turret_angle_degrees())
    ...
    def shooting(self,initial_call):
        """Actively shooting"""
        self.hub_tracker.setEnabled(True)
        if not self.is_shooting or not self._get_shooter_state_is_target_state_default() or self.is_drivetrain_moving():
            self.next_state("targeting")
            return
    ...
    def _get_shooter_state_is_target_state(self,turret_tolerance_degrees: float,
        ...
        turret_error = abs(self.hub_tracker.get_predictive_turret_target_angle_degrees() - self.turret.get_turret_angle())
        hood_error = abs(self.hub_tracker.get_target_hood_angle_degrees() - self.hood.get_measured_angle_degrees())
        flywheel_error = abs(self.hub_tracker.get_target_flywheel_speed_rps() - self.flywheel.flywheel_encoder.get_velocity().value)
    ...
    def _shooter_state_vs_target_state(self,turret_tolerance_degrees: float,
                                           hood_tolerance_degrees: float,
                                           flywheel_tolerance_rotations_per_second: float) -> str:
        turret_error = abs(self.hub_tracker.get_predictive_turret_target_angle_degrees() - self.turret.get_turret_angle())
        hood_error = abs(self.hub_tracker.get_target_hood_angle_degrees() - self.hood.get_measured_angle_degrees())
        flywheel_error = abs(self.hub_tracker.get_target_flywheel_speed_rps() - self.flywheel.flywheel_encoder.get_velocity().value)
        ...
        return f"Turret error is {te}. Hood error is {he}. Flywheel error is {fe}."
    
    def is_drivetrain_moving(self) -> bool:
        return self.get_total_drivetrain_speed() > 0.1
    def get_total_drivetrain_speed(self) -> float:
        return math.sqrt((self.drivetrain.get_robot_speed().vx**2)+(self.drivetrain.get_robot_speed().vy**2))

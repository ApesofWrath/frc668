...
class DepotNeutralZoneShoot(AutonomousStateMachine):
    """Start on the outpost side of the ramp, drive to the neutral zone, intake fuel, come back and shoot."""
    MODE_NAME = "get_neutral_zone_shoot_starting_outpost_side"

    AutoHelper: AutoHelper.AutoHelper
    intake_deployer: intake.IntakeDeployer

    def __init__(self):
        self.trajectory = choreo.load_swerve_trajectory("OutpostSideBump_IntakeNeutralZone_Shoot")

    def on_enable(self):
        self.AutoHelper.reset("OutpostSideBump_IntakeNeutralZone_Shoot",True)
        super().on_enable()

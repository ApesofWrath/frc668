...
def mock_turret():
    ...
    turret.measured_angle_degrees.return_value = 0.0
    return turret
...
def mock_hood():
    ...
    hood = mock.MagicMock()
    hood.measured_angle_degrees.return_value = 0.0
    return hood
...
def mock_flywheel():
    ...
    flywheel.measured_speed_rps.return_value = 20.0
...
def mock_drivetrain():
    ...
    drivetrain.robot_speeds.return_value = chassis_speeds
    return drivetrain
...
def mock_hub_tracker():
    ...
    hub_tracker.target_turret_angle_degrees.return_value = 0.0
    hub_tracker.target_hood_angle_degrees.return_value = 0.0
    hub_tracker.target_flywheel_speed_rps.return_value = 20.0
    return hub_tracker
...
class TestShooter:
    ...
    # -------------------------------------------------------------------------
    # set_driver_wants_feed tests
    # -------------------------------------------------------------------------

    def test_set_driver_wants_feed_true(self, shooter_sm):
        """set_driver_wants_feed(True) should set the flag to True."""
        shooter_sm.set_driver_wants_feed(True)
        assert shooter_sm._driver_wants_feed is True

    def test_set_driver_wants_feed_false(self, shooter_sm):
        """set_driver_wants_feed(False) should set the flag to False."""
        shooter_sm.set_driver_wants_feed(True)
        shooter_sm.set_driver_wants_feed(False)
        assert shooter_sm._driver_wants_feed is False
    ...
    def test_idling_to_targeting_when_driver_wants_feed(
        ...
        mock_hub_tracker.track_position.assert_called()
        mock_hub_tracker.track_speed.assert_called()
    ...
    def test_idling_stays_idling_when_driver_does_not_want_feed(
        ...
        assert shooter_sm.current_state == "idling"
        mock_hub_tracker.track_position.assert_called_with(True)
        mock_hub_tracker.track_speed.assert_called_with(False)
        mock_hopper.set_enabled.assert_called_with(False)
        mock_indexer.set_enabled.assert_called_with(False)
    ...
    def test_idling_sets_flywheel_to_default_speed(
        ...
        mock_hub_tracker.set_target_flywheel_speed_rps.assert_called_with(
    ...
    def test_targeting_to_shooting_when_ready_and_stationary(
        ...
        mock_flywheel.measured_speed_rps.return_value = 20.0  # Within tolerance

        # Robot is stationary
        mock_drivetrain.robot_speeds.return_value.vx = 0.0
        mock_drivetrain.robot_speeds.return_value.vy = 0.0
    ...
    def test_targeting_stays_targeting_when_not_ready(
        ...
        mock_hub_tracker.target_turret_angle_degrees.return_value = 45.0
        mock_turret.measured_angle_degrees.return_value = 0.0  # Far from target
    ...
    def test_targeting_stays_targeting_when_robot_moving(
        ...
        # Shooter is ready
        mock_hub_tracker.target_turret_angle_degrees.return_value = 0.0
        mock_turret.measured_angle_degrees.return_value = 0.0
        mock_hub_tracker.target_hood_angle_degrees.return_value = 0.0
        mock_hood.measured_angle_degrees.return_value = 0.0
        mock_hub_tracker.target_flywheel_speed_rps.return_value = 20.0
        mock_flywheel.measured_speed_rps.return_value = 20.0

        # Robot is moving
        mock_drivetrain.robot_speeds.return_value.vx = 1.0
        mock_drivetrain.robot_speeds.return_value.vy = 0.0
    ...
    def test_shooting_to_targeting_when_shooter_becomes_not_ready(
        ...
        mock_turret.measured_angle_degrees.return_value = 15.0
        shooter_sm.engage()
        shooter_sm.execute()
    ...
    def test_shooting_stays_shooting_when_robot_starts_moving(
        ...
        mock_drivetrain.robot_speeds.return_value.vx = 0.5
        shooter_sm.engage()
        shooter_sm.execute()
    ...
    def test_shooting_enables_hopper_and_indexer(
        ...
        # Note: targeting calls set_enabled(False) before next_state_now,
        # then shooting calls set_enabled(True). Both run in same execute.
        mock_hopper.set_enabled.assert_any_call(True)
        mock_indexer.set_enabled.assert_any_call(True)
    ...
    def test_shooting_tracks_position_and_speed(
        ...
        mock_hub_tracker.track_position.assert_called_with(True)
        mock_hub_tracker.track_speed.assert_called_with(True)
    ...
    def test_shooter_is_ready_when_within_all_tolerances(
        ...
        mock_hub_tracker.target_turret_angle_degrees.return_value = 10.0
        mock_turret.measured_angle_degrees.return_value = 12.0  # Error = 2 < 6
        mock_hub_tracker.target_hood_angle_degrees.return_value = 5.0
        mock_hood.measured_angle_degrees.return_value = 5.5  # Error = 0.5 < 3
        mock_hub_tracker.target_flywheel_speed_rps.return_value = 30.0
        mock_flywheel.measured_speed_rps.return_value = 28.0  # Error = 2 < 5

        assert shooter_sm._shooter_is_ready() is True
    ...
    def test_shooter_is_not_ready_when_turret_out_of_tolerance(
        ...
        mock_hub_tracker.target_turret_angle_degrees.return_value = 10.0
        mock_turret.measured_angle_degrees.return_value = 3.0  # Error = 7 > 6
        mock_hub_tracker.target_hood_angle_degrees.return_value = 5.0
        mock_hood.measured_angle_degrees.return_value = 5.0
        mock_hub_tracker.target_flywheel_speed_rps.return_value = 30.0
        mock_flywheel.measured_speed_rps.return_value = 30.0

        assert shooter_sm._shooter_is_ready() is False
    ...
    def test_shooter_is_not_ready_when_hood_out_of_tolerance(
        ...
        mock_hub_tracker.target_turret_angle_degrees.return_value = 10.0
        mock_turret.measured_angle_degrees.return_value = 10.0
        mock_hub_tracker.target_hood_angle_degrees.return_value = 5.0
        mock_hood.measured_angle_degrees.return_value = 8.5  # Error = 3.5 > 3
        mock_hub_tracker.target_flywheel_speed_rps.return_value = 30.0
        mock_flywheel.measured_speed_rps.return_value = 30.0
    ...
    def test_shooter_is_not_ready_when_flywheel_out_of_tolerance(
        ...
        mock_hood.measured_angle_degrees.return_value = 5.0
        mock_hub_tracker.target_flywheel_speed_rps.return_value = 30.0
        mock_flywheel.measured_speed_rps.return_value = 24.0  # Error = 6 > 5
    ...
    def test_shooter_is_ready_at_exact_tolerance_boundary(
        ...
        mock_hub_tracker.target_turret_angle_degrees.return_value = 10.0
        mock_turret.measured_angle_degrees.return_value = 16.0  # Error = 6 <= 6
        mock_hub_tracker.target_hood_angle_degrees.return_value = 5.0
        mock_hood.measured_angle_degrees.return_value = 8.0  # Error = 3 <= 3
        mock_hub_tracker.target_flywheel_speed_rps.return_value = 30.0
        mock_flywheel.measured_speed_rps.return_value = 25.0  # Error = 5 <= 5

        assert shooter_sm._shooter_is_ready() is True

    # -------------------------------------------------------------------------
    # _robot_is_moving tests
    # -------------------------------------------------------------------------
    ...
    def test_robot_is_not_moving_when_stationary(
        ...
        assert shooter_sm._robot_is_moving() is False

    def test_robot_is_moving_when_vx_exceeds_threshold(
        self, shooter_sm, mock_drivetrain
    ):
        """Robot is moving when vx exceeds the threshold."""
        mock_drivetrain.robot_speeds.return_value.vx = 0.15
        mock_drivetrain.robot_speeds.return_value.vy = 0.0

        assert shooter_sm._robot_is_moving() is True
    ...
    def test_robot_is_moving_when_vy_exceeds_threshold(
        ...
        mock_drivetrain.robot_speeds.return_value.vx = 0.0
        mock_drivetrain.robot_speeds.return_value.vy = 0.15
    ...
    def test_robot_is_moving_with_combined_velocity(
        ...
        mock_drivetrain.robot_speeds.return_value.vx = 0.08
        mock_drivetrain.robot_speeds.return_value.vy = 0.08
    ...
    def test_robot_is_not_moving_at_threshold(
        ...
        mock_drivetrain.robot_speeds.return_value.vx = 0.1
        mock_drivetrain.robot_speeds.return_value.vy = 0.0
    ...
    def test_robot_is_moving_with_custom_threshold(
        ...
        """Custom threshold can be passed to _robot_is_moving."""
        mock_drivetrain.robot_speeds.return_value.vx = 0.2
        mock_drivetrain.robot_speeds.return_value.vy = 0.0

        assert shooter_sm._robot_is_moving(speed_threshold_mps=0.3) is False
        assert shooter_sm._robot_is_moving(speed_threshold_mps=0.1) is True
    ...
    def test_get_shooter_is_within_with_all_within_tolerance(
        ...
        """_get_shooter_is_within returns True when all within custom tolerances."""
        mock_hub_tracker.target_turret_angle_degrees.return_value = 10.0
        mock_turret.measured_angle_degrees.return_value = 15.0  # Error = 5
        mock_hub_tracker.target_hood_angle_degrees.return_value = 5.0
        mock_hood.measured_angle_degrees.return_value = 7.0  # Error = 2
        mock_hub_tracker.target_flywheel_speed_rps.return_value = 30.0
        mock_flywheel.measured_speed_rps.return_value = 26.0  # Error = 4

        # All within wider tolerances
        result = shooter_sm._get_shooter_is_within(
    ...
    def test_get_shooter_is_within_with_one_out_of_tolerance(
        ...
        """_get_shooter_is_within returns False when one component is out of tolerance."""
        mock_hub_tracker.target_turret_angle_degrees.return_value = 10.0
        mock_turret.measured_angle_degrees.return_value = 15.0  # Error = 5
        mock_hub_tracker.target_hood_angle_degrees.return_value = 5.0
        mock_hood.measured_angle_degrees.return_value = 5.0  # Error = 0
        mock_hub_tracker.target_flywheel_speed_rps.return_value = 30.0
        mock_flywheel.measured_speed_rps.return_value = 30.0  # Error = 0
